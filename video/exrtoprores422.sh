...
# exrtoprores422.sh – fast EXR → PNG → ProRes 422 HQ MOV converter
# ProRes 422 has no alpha channel: source alpha is composited over black
# so transparent regions encode correctly (no fringe / no garbage).
...
echo "🟢  Starting EXR-to-MOV (ProRes 422 HQ) script …"
...
process_dir() {
  ...
  echo "🎨 Converting ${#exrs[@]} EXRs to PNGs (alpha composited over black) …"
  ...
  echo "🎞  Encoding ProRes 422 HQ → $out_mov"
