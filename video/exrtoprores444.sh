...
# exrtoprores444.sh – fast EXR → PNG → ProRes 4444 MOV converter
# Preserves alpha channel (ProRes 4444 supports alpha).
...
echo "🟢  Starting EXR-to-MOV (ProRes 4444) script …"
...
while (( $# )); do
  case "$1" in
    ...
    -res)  res_override="$2"; shift 2 ;;
    -j)    PARALLEL_JOBS="$2"; shift 2 ;;
    -meta) INCLUDE_META=true; shift ;;
    --)    shift; while (( $# )); do DIRS+=("$1"); shift; done ;;
...
process_dir() {
  ...
  local exrs=( *.exr )
  if (( ${#exrs[@]} == 0 )); then
    echo "No EXR files in $DIR — skipping."
    popd >/dev/null
    return 0
  fi

  local first_exr="${exrs[0]}" res width height fps
  fps=$(exr_video_fps "$first_exr" "$fps_override") || { popd >/dev/null; return 1; }
  ...
  echo "🎨 Converting ${#exrs[@]} EXRs to PNGs (RGBA) …"
  local PAR_CMD=( parallel --halt soon,fail=1 --line-buffer )
  ...
  if $INCLUDE_META; then
    ...
    "${PAR_CMD[@]}" --colsep '\|' '
      ...
      CH_ARGS=$(exr_channel_args "$f" opaque) || exit 1
      oiiotool "$f" --ch "$CH_ARGS" --colorconvert "ACES - ACEScg" "Output - sRGB" --attrib oiio:UnassociatedAlpha 1 \
  ...
  exr_video_link_frames "$tmpdir" || { popd >/dev/null; popd >/dev/null; return 1; }

  local out_base out_mov
  out_base=$(basename "$first_exr" .exr | sed -E 's/\.[0-9]+$//')
  out_mov="${out_base}_prores444.mov"
  echo "🎞  Encoding ProRes 4444 (with alpha) → $out_mov"

  # Read numbered PNGs directly for parallel decoding and exact frame timing.
  ffmpeg -y -loglevel error \
         -framerate "$fps" -start_number 0 -i "frames/%08d.png" \
         -c:v prores_ks -profile:v 4 -pix_fmt yuva444p10le \
         -fps_mode passthrough \
         -s "$res" \
         "$out_mov" || { echo "ERROR: FFmpeg failed in $DIR"; popd >/dev/null; popd >/dev/null; return 1; }

  popd >/dev/null

  mv "$tmpdir/$out_mov" "$ORIG_CWD/"
  rm -rf "$tmpdir"
  echo "✅  Done $DIR → $ORIG_CWD/$out_mov"
